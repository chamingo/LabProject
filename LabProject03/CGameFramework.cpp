...
void CGameFramework::CreateSwapChain()
{
	...
	m_nWndClientHeight = rcClient.bottom - rcClient.top; DXGI_SWAP_CHAIN_DESC1 dxgiSwapChainDesc;
...
void CGameFramework::CreateDirect3DDevice()
{
	...
	**)&m_pd3dFence);
	...
	//뷰포트를 주 윈도우의 클라이언트 영역 전체로 설정한다. m_d3dScissorRect = { 0, 0, m_nWndClientWidth, m_nWndClientHeight };
	//씨저 사각형을 주 윈도우의 클라이언트 영역 전체로 설정한다. if (pd3dAdapter) pd3dAdapter->Release();
}
...
void CGameFramework::CreateCommandQueueAndList()
{
	...
	d3dCommandQueueDesc.Type = D3D12_COMMAND_LIST_TYPE_DIRECT; HRESULT hResult = m_pd3dDevice->CreateCommandQueue(&d3dCommandQueueDesc,
...
void CGameFramework::CreateRenderTargetView()
{
	...
	for (UINT i = 0; i < m_nSwapChainBuffers; i++)
	{
		HRESULT hResult = m_pdxgiSwapChain->GetBuffer(i, __uuidof(ID3D12Resource), (void
			**)&m_ppd3dRenderTargetBuffers[i]); m_pd3dDevice->CreateRenderTargetView(m_ppd3dRenderTargetBuffers[i], NULL,
...
void CGameFramework::FrameAdvance()
{
	...
	::ZeroMemory(&d3dResourceBarrier, sizeof(D3D12_RESOURCE_BARRIER)); d3dResourceBarrier.Type = D3D12_RESOURCE_BARRIER_TYPE_TRANSITION;
	...
	내용이 전면버퍼로 옮겨지고 렌더 타겟 인덱스가 바뀔 것이다.*/
